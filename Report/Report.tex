...
For this study, we used the dataset contributed by Mumtaz et al. (\cite{Mumtaz2017WaveletMDD}) and is publicly available at (\cite{FigshareMDD2016}).  34 MDD patients (17 females and 17 males) participated in this study and fulfilled the globally accepted Diagnostic and Statistical Manual-IV (DSM-IV) depression criteria. In addition, a control group of 30 healthy individuals (9 females and 21 men) were recruited. The healthy people were examined for signs of mental disorders and found to be free of such problems.
...
EEG data is highly suited for GC analysis due to its high temporal resolution, quick sampling, and often stochastic character (\cite{Barnett2014MVGC}). In this work, we use the Multivariate Granger Causality (MVGC) (\cite{Barnett2014MVGC}) toolbox for GC estimation. The algorithm computes MVGC using time–series data in both the frequency and time domains. It eliminates separate full and reduced regressions, which are a typical cause of statistical inaccuracy and computational inefficiency, therefore improving upon traditional techniques for GC inference (\cite{Barnett2014MVGC}). 
...
where $p$ is the model order. In this experiment, the Akaike Information Criterion (AIC) was used to establish the proper model order, helping to avoid under–representation or overfitting of the mode. The AIC for $n$ variables is defined as
...
In this case, the addition of $Z$ in both regressions is considered as its combined influence. $F_{Y \rightarrow X|Z}$ is interpreted as the extent to which the previous values of $Y$ assist in predicting $X$, beyond the amount to which $X$ is predicted by its own history and by $Z$'s history (\cite{Barnett2014MVGC}).

The corresponding p-values from Chi-square significance tests were extracted and used as the GC measure between the two channels. This produces a 19×19 GC matrix.

\subsection{Multi-Layer Perceptron}
A multi-layer perceptron (MLP) is a simple artificial neural network of sequential transformations and activations to replicate the connection of neurons in the brain. We use this model as a baseline to assess whether a complex convolutional neural network (CNN) model is necessary. 

We first process the 5-min EEG data into 1 second segments, creating 3000 independent samples for each subject. Some of these samples are discarded due to a high presence of noise. Then, we use an 80/20 train-test split on the data by subject, reserving 80\% of MDD and HC subjects for model training and the remaining for testing. That is, the training dataset includes 28 MDD and 24 HC subjects, while the testing dataset includes the remaining 7 MDD and 6 HC subjects. 

The architecture of the neural network is easily tunable, from the activation functions used between layers, to the size and depth of each hidden (intermediate) layer. After tuning the architecture to optimize for model performance,  the following architecture was selected. 
...
\subsection{MLP Parameter Tuning}

\subsection{CNN Parameter Tuning}
